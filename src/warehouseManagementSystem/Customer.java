...
public class Customer extends AbstractUser{
    ...
    public boolean giveProductOrder(String productName,int pieces){
        if(this.getStore().getProductStock().containsKey(productName)) {
            int productQuantity = Integer.parseInt(this.getStore().getProductStock().get(productName));

            if (productQuantity > 0 ) {
                int newPiece=Integer.parseInt(this.getStore().getProductStock().get(productName))-pieces;
                if(newPiece<0)
                    return false;
                this.getStore().getProductStock().put(productName,String.valueOf(newPiece));
                return true;
            }
        }
    ...
    public void customerMenu(Customer customer) {
        ...
        try {
            while (state) {
                ...
                System.out.println(" *  Give Product Order              => 2     *");
                ...
                switch (choose) {
                    ...
                    case 2:{
                        ...
                        if(!customer.giveProductOrder(name,Integer.parseInt(quantity))){
